...
public class ChamadoDetailViewModel : BaseViewModel
{
    ...
    private async Task LoadComentariosAsync(bool showErrors)
    {
        ...
        try
        {
            ...
            if (comentarios != null)
            {
                foreach (var comentario in comentarios.OrderBy(c => c.DataHora == default ? c.DataCriacao : c.DataHora))
                {
    ...
    private void InserirComentarioOrdenado(ComentarioDto comentario)
    {
        ...
        var dataComparacao = comentario.DataHora == default ? comentario.DataCriacao : comentario.DataHora;
        var index = 0;

        foreach (var existente in Comentarios)
        {
            var existenteData = existente.DataHora == default ? existente.DataCriacao : existente.DataHora;
