...
public class ChamadosController : ControllerBase
{
    ...
    public async Task<IActionResult> CriarChamado([FromBody] CriarChamadoRequestDto request)
    {
        ...
        // Busca o Status "Aberto" dinamicamente
        var statusAberto = await _context.Status.FirstOrDefaultAsync(s => s.Nome == "Aberto");
        if (statusAberto == null)
        {
            return StatusCode(500, "Status 'Aberto' não encontrado no banco de dados.");
        }
        ...
        // ✅ CORREÇÃO: Recarregar chamado com navegações para retornar DTO completo
...
public async Task<IActionResult> GetChamados([FromQuery] int? statusId, [FromQuery] int? tecnicoId, [FromQuery] int? solicitanteId, [FromQuery] int? prioridadeId, [FromQuery] string? termoBusca)
{
    ...
    try
    {
        ...
        // 7. Executa a consulta com todos os filtros e retorna objetos completos para o Mobile
        ...
        _logger.LogInformation("GetChamados - Consulta finalizada. Resultados encontrados: {Count}", chamados.Count);
        return Ok(chamados); 
    }
    catch (Exception ex)
    {
...
public async Task<IActionResult> AnalisarChamado([FromBody] AnalisarChamadoRequestDto request)
{
    ...
    try
    {
        ...
        // ✅ CORREÇÃO: Recarregar chamado com navegações para retornar DTO completo ao Mobile
        var chamadoCompleto = await _context.Chamados
            .Include(c => c.Categoria)
            .Include(c => c.Prioridade)
            .Include(c => c.Status)
        ...
        // 5. Retorna o chamado que foi CRIADO no banco (com seu novo ID e navegações populadas)
